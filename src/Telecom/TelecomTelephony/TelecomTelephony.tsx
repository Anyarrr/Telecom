...
export const TelecomTelephony: React.FC = () => {
  const [searchQuery, setSearchQuery] = useState('');
  const [selectedCategory, setSelectedCategory] = useState<string>('all');

  // Данные тарифов (основные направления)
  ...
  const filteredTariffs = useMemo(() => {
    ...
    if (selectedCategory !== 'all') {
      filtered = filtered.filter(tariff => {
        const dir = tariff.direction.toLowerCase();
        switch (selectedCategory) {
          case 'russia':
            return dir.includes('россия');
          case 'europe':
            return dir.includes('греция') || dir.includes('нидерланды') || dir.includes('бельгия') ||
                   dir.includes('франция') || dir.includes('испания') || dir.includes('португалия') ||
                   dir.includes('кипр') || dir.includes('болгария') || dir.includes('венгрия') ||
                   dir.includes('литва') || dir.includes('латвия') || dir.includes('эстония') ||
                   dir.includes('молдова') || dir.includes('беларусь') || dir.includes('украина') ||
                   dir.includes('сербия') || dir.includes('хорватия') || dir.includes('словения') ||
                   dir.includes('италия') || dir.includes('румыния') || dir.includes('швейцария') ||
                   dir.includes('чехия') || dir.includes('австрия') || dir.includes('великобритания') ||
                   dir.includes('швеция') || dir.includes('норвегия') || dir.includes('польша') ||
                   dir.includes('германия');
          case 'asia':
            return dir.includes('китай') || dir.includes('япония') || dir.includes('корея') ||
                   dir.includes('вьетнам') || dir.includes('тайвань') || dir.includes('турция') ||
                   dir.includes('индия') || dir.includes('пакистан') || dir.includes('афганистан') ||
                   dir.includes('оман') || dir.includes('оаэ') || dir.includes('израиль') ||
                   dir.includes('монголия') || dir.includes('иран') || dir.includes('таджикистан') ||
                   dir.includes('туркменистан') || dir.includes('азербайджан') || dir.includes('грузия') ||
                   dir.includes('кыргызстан') || dir.includes('узбекистан') || dir.includes('казахстан');
          case 'america':
            return dir.includes('сша') || dir.includes('канада') || dir.includes('куба');
          case 'other':
            return dir.includes('австралия') || dir.includes('индонезия') || dir.includes('новая зеландия') ||
                   dir.includes('абхазия') || dir.includes('бангладеш') || dir.includes('мьянма') ||
                   dir.includes('ливан') || dir.includes('палестина');
          default:
            return true;
        }
      });
    ...
    if (searchQuery) {
      ...
      filtered = filtered.filter(tariff =>
        tariff.pattern.toLowerCase().includes(query) ||
        tariff.direction.toLowerCase().includes(query) ||
        tariff.price.includes(query)
      );
    }
  ...
  return (
    <section id="telephony" className="min-h-screen py-32 px-6 relative overflow-hidden bg-black">
      ...
      <div className="max-w-7xl mx-auto relative z-10 w-full">
        ...
        <div className="text-center mb-16 space-y-6">
          ...
          <p className="text-gray-400 text-sm md:text-base max-w-2xl mx-auto leading-relaxed opacity-80">
            Актуальные тарифы на IP-телефонию. Минимальная длительность сеанса связи составляет 1 сек.
          </p>
        </div>
        ...
        <div className="mb-12 p-6 bg-white/5 border border-white/10 rounded-2xl backdrop-blur-sm">
          <div className="flex items-start gap-4">
            <Info className="w-6 h-6 text-green-500 flex-shrink-0 mt-1" />
            <div className="space-y-3 text-sm text-gray-400">
              <p><strong className="text-white">Общая информация:</strong></p>
              <ul className="list-disc list-inside space-y-1 ml-4">
                <li>Минимальная длительность сеанса связи составляет 1 сек., дальнейшее приращение длительности сеанса связи осуществляется временными интервалами по 1 сек.</li>
                <li>При тарификации используются исключительно статистические данные Исполнителя.</li>
                <li>После завершения сеанса связи с лицевого счета Заказчика списывается фактическая стоимость сеанса связи, равная длительности сеанса связи в минутах, умноженной на цену Направления.</li>
                <li>Началом сеанса связи считается момент получения подтверждения на SIP-сообщение ACK.</li>
                <li>Стоимость остальных услуг оказываемых Исполнителем заказчику списывается с лицевого счета в начале учетного периода действия услуги.</li>
              </ul>
            </div>
          </div>
        </div>
        ...
        <div className="mb-8 space-y-6">
          ...
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Поиск по номеру, направлению или цене..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-12 pr-4 py-4 bg-white/5 border border-white/10 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:border-green-500/50 focus:bg-white/10 transition-all"
            />
          </div>
              ...
              <button
                ...
                className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                  selectedCategory === category.id
                    ...
                    : 'bg-white/5 text-gray-400 border border-white/10 hover:border-green-500/50 hover:text-white'
                }`}
              >
                {category.label}
              </button>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-white/10 bg-white/5">
                  <th className="px-6 py-4 text-left text-xs font-black uppercase tracking-widest text-green-500">Номер (шаблон)</th>
                  <th className="px-6 py-4 text-left text-xs font-black uppercase tracking-widest text-green-500">Направление</th>
                  <th className="px-6 py-4 text-right text-xs font-black uppercase tracking-widest text-green-500">Цена, руб (без НДС)</th>
                </tr>
              </thead>
              <tbody>
                {filteredTariffs.length > 0 ? (
                  filteredTariffs.map((tariff, index) => (
                    ...
                    >
                      <td className="px-6 py-4 text-white font-mono text-sm group-hover:text-green-500 transition-colors">
                        {tariff.pattern}
                      </td>
                      <td className="px-6 py-4 text-gray-300 text-sm group-hover:text-white transition-colors">
                        {tariff.direction}
                      </td>
                      <td className="px-6 py-4 text-right text-white font-bold text-sm group-hover:text-green-500 transition-colors">
                        {tariff.price}
                      </td>
                    </tr>
                  ))
        ...
        <div className="mt-6 text-center text-gray-500 text-sm">
          Найдено тарифов: <span className="text-green-500 font-bold">{filteredTariffs.length}</span> из {tariffs.length}
        </div>
      </div>
    </section>
  );
